...
* The sky background is assumed to be a single value, however as discussed in :doc:`the global sky <background>` belies this assumption.  Employing a realistic :term:`global-sky image` with a scale factor (:math:`\alpha`) by modifying the source/uncertainty equations to have :math:`B\rightarrow \alpha\,B_{x,y}`, where :math:`\alpha` becomes the tunable parameter to be set by the user.  The current implementation will give the illusion of a constant S/N over the detector, but the deviations from constant will depend on the how adopted level compares to the (large-scale) variations in the :term:`global-sky image`. Therefore this may introduce small systematic biases based on the position of the sources.
