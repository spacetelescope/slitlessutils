...
class Source(list):
    ...
    def __init__(self,img,seg,hdr,local_back=True,backsize=5,nsig=(5,5),
                 whttype='pixels',zeropoint=26.,grpid=0):
        """
        Initializer
        ...
        if self.npixels>0:
            ...
            if self.segid < 0:
                ...
                for i,(xx,yy,ww) in enumerate(zip(x,y,w),start=1):
                    ...
                    self.append(reg)
                    #self.spectralregions.append(reg)
            elif self.segid >0:
                #self.spectralregions.append(SpectralRegion(x,y,w,self.segid,0,ltv=self.ltv))
                self.append(SpectralRegion(x,y,w,self.segid,0,ltv=self.ltv))
            else:
                LOGGER.warning("segid==0 is ignored")
                
            
    #def __bool__(self):
    #    """
    #    Override the bool() function based on if there are spectral regions
    #    """
    #    return bool(self.spectralregions)

    #def __getitem__(self,k):
    #    """
    #    Method to emulate the dict behavior
    #    """
    #
