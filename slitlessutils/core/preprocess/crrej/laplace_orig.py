    ...
    with fits.open(filename,mode=mode) as hdul:
        ...
        for hdu in hdul:
            if hdu.header.get('EXTNAME','primary')=='SCI':
                ...
                reg=measure.label(bpx)
                crs=indices.reverse(reg)
                ...
                #crs=measure.regionprops(reg)
                #for cr in crs[1:]:
                #    if cr.area < minpix or cr.area > maxpix:
                #        print(cr.area,cr.label)
                #xr=cr.coords[:,1]
                #        yr=cr.coords[:,0]
                #        print(bpx[yr,xr])
                #        bpx[cr.coords]=False
                ...
                #axes[0].imshow(bpx.astype(int))
                #axes[1].imshow(reg)
                #plt.show()
                #kdjf
                
                # eliminate small events (this is usually noisy pixels
                # that are above SN cut, which are typically too small
                # to be considered as cosmic rays)
                #if minpix is not None and minpix > 0:
                #    crs=measure.label(bpx)#
                #npix=np.bincount(crs.ravel())
                #    print(npix[0:100])
                #    print(len(npix),sum(npix))
#
#                    ljdf
#
#                    
#                    bpx = morphology.remove_small_objects(crs,minpix) !=0
                ...
                if g[0].size>0:
                    ...
                    if interp:
                        ...
                        for cr in measure.regionprops(measure.label(bpx)):
                            y0=max(cr.bbox[0]-3,0)
                            x0=max(cr.bbox[1]-3,0)
                            y1=min(cr.bbox[2]+3,sci.shape[0]-1)
                            x1=min(cr.bbox[3]+3,sci.shape[1]-1)
                            ...
                            #with warnings.catch_warnings():
                            #    warnings.simplefilter('error')
                            hdul[('SCI',extver)].data[by+y0,bx+x0]=spline(subx[subbpx],suby[subbpx],grid=False)
